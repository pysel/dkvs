// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.31.0
// 	protoc        v3.21.9
// source: dkvs/partition/rpc.proto

package partition

import (
	prototypes "github.com/pysel/dkvs/prototypes"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

var File_dkvs_partition_rpc_proto protoreflect.FileDescriptor

var file_dkvs_partition_rpc_proto_rawDesc = []byte{
	,
}

var file_dkvs_partition_rpc_proto_goTypes = []interface{}{
	(*prototypes.SetRequest)(nil),           // 0: dkvs.message.SetRequest
	(*prototypes.GetRequest)(nil),           // 1: dkvs.message.GetRequest
	(*prototypes.DeleteRequest)(nil),        // 2: dkvs.message.DeleteRequest
	(*prototypes.SetHashrangeRequest)(nil),  // 3: dkvs.message.SetHashrangeRequest
	(*prototypes.SetResponse)(nil),          // 4: dkvs.message.SetResponse
	(*prototypes.GetResponse)(nil),          // 5: dkvs.message.GetResponse
	(*prototypes.DeleteResponse)(nil),       // 6: dkvs.message.DeleteResponse
	(*prototypes.SetHashrangeResponse)(nil), // 7: dkvs.message.SetHashrangeResponse
}
var file_dkvs_partition_rpc_proto_depIdxs = []int32{
	0, // 0: dkvs.partition.PartitionService.Set:input_type -> dkvs.message.SetRequest
	1, // 1: dkvs.partition.PartitionService.Get:input_type -> dkvs.message.GetRequest
	2, // 2: dkvs.partition.PartitionService.Delete:input_type -> dkvs.message.DeleteRequest
	3, // 3: dkvs.partition.PartitionService.SetHashrange:input_type -> dkvs.message.SetHashrangeRequest
	4, // 4: dkvs.partition.PartitionService.Set:output_type -> dkvs.message.SetResponse
	5, // 5: dkvs.partition.PartitionService.Get:output_type -> dkvs.message.GetResponse
	6, // 6: dkvs.partition.PartitionService.Delete:output_type -> dkvs.message.DeleteResponse
	7, // 7: dkvs.partition.PartitionService.SetHashrange:output_type -> dkvs.message.SetHashrangeResponse
	4, // [4:8] is the sub-list for method output_type
	0, // [0:4] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_dkvs_partition_rpc_proto_init() }
func file_dkvs_partition_rpc_proto_init() {
	if File_dkvs_partition_rpc_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_dkvs_partition_rpc_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   0,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_dkvs_partition_rpc_proto_goTypes,
		DependencyIndexes: file_dkvs_partition_rpc_proto_depIdxs,
	}.Build()
	File_dkvs_partition_rpc_proto = out.File
	file_dkvs_partition_rpc_proto_rawDesc = nil
	file_dkvs_partition_rpc_proto_goTypes = nil
	file_dkvs_partition_rpc_proto_depIdxs = nil
}
